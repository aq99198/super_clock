...
void TaskB(void)
{
    while (1)
    {
		IOCTL(LED, 1, LED_TRIGGER, NULL);
        OSWait(K_TMO,10);
		//IOCTL(UART, 0, UART_SET_STR, (uint16)"TaskB\r\n");
    }
}
...
void TaskC(void)
{
	...
	timer.time_ms = 5000; // ms
	timer.function = timer_notify0;
	timer_init(&timer);
	
    while (1)
    {
		IOCTL(LED, 2, LED_TRIGGER, NULL);
        OSWait(K_TMO,15);
    }
}
